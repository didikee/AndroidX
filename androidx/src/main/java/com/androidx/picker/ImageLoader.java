...
public class ImageLoader extends AbsMediaLoader {
    ...
    private ArrayList<MediaFolder> load(Context context) {
        ...
        final int index_WIDTH = projections.indexOf(MediaStore.MediaColumns.WIDTH);
        final int index_HEIGHT = projections.indexOf(MediaStore.MediaColumns.HEIGHT);
        ...
        while (cursor.moveToNext()) {
            //查询数据
            String id = getCursorString(cursor,index_ID);
            String displayName = getCursorString(cursor,index_DISPLAY_NAME);
            String mimeType = getCursorString(cursor,index_MIME_TYPE);
            ...
            if (isAndroid10) {
                if (externalStorageLegacy) {
                    data = getCursorString(cursor,index_DATA);
                } else {
                    relativePath = getCursorString(cursor,index_RELATIVE_PATH);
                }
            } else {
                data = getCursorString(cursor,index_DATA);
            }
            ...
            // 这些是公用的参数
            long size = getCursorLong(cursor,index_SIZE);
            long dateAdded = getCursorLong(cursor,index_DATE_ADDED);
            long dateModified = getCursorLong(cursor,index_DATE_MODIFIED);
            ...
            // 添加专有参数
            // 这些和类型绑定的
            int width = getCursorInt(cursor,index_WIDTH);
            int height = getCursorInt(cursor,index_HEIGHT);
            //封装实体
    ...
    protected int getCursorInt(Cursor cursor, int columnIndex) {
        if (columnIndex >= 0) {
            try {
    ...
    protected String getCursorString(Cursor cursor, int columnIndex) {
        if (columnIndex >= 0) {
            try {
    ...
    protected Long getCursorLong(Cursor cursor, int columnIndex) {
        if (columnIndex >= 0) {
            try {
